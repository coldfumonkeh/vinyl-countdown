      ...
      <div class="col">
        <div id="sort-bar" class="row gy-2 gx-3 align-items-center mt-4">
          ...
          <div class="col-auto" v-if="totalRecords">
            <div class="col-auto gy-2 gx-3 align-items-right mt-3">Total Records: {{ filteredRecords.length }}/{{ totalRecords }}</div>
          </div>
        </div>
      </div>
    </div>
            ...
            <div class="col-8">
              <div class="card-body">
                <div class="row">
                  <div class="col-12">
                    <p class="card-title">
                      <router-link :to="{ path: '/r/' + record.id }">{{ record.title }} - <small>{{ record.artists_sort }}</small></router-link>
                    </p>
  ...
  computed: {
    ...
    filteredRecords() {
      ...
      if (this.searchValue) {
        ...
        tempRecords = tempRecords.filter(item => item.searchText.includes(query))
      }
      ...
      tempRecords = tempRecords.sort((a, b) => {
        ...
        const fa = (a.artists[0] && a.artists[0].name ? a.artists[0].name : '').toLowerCase()
        const fb = (b.artists[0] && b.artists[0].name ? b.artists[0].name : '').toLowerCase()
        if (fa < fb) return -1
        if (fa > fb) return 1
        return 0
      })
